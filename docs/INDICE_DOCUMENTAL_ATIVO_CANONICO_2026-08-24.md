...
| `evidence/EVD_POST_E13_SOAK_MEMORIA_GPU_CONTROLADO_20260913.md` | ATIVO / EVIDÊNCIA `PASS` memória; `PASS_CONTROLLED_GPU_WORKLOAD`; viewport `NOT_APPLICABLE` | soak controlado de 26 workloads com 250 ciclos de memória por carga e workload CUDA de 20 s com 38 amostras, 36.408 operações e pico de 91% na RTX 3070 Ti; contador do QGraphicsView permanece não exposto | governança, decisão de soak, benchmark P2D-05, relatório GPU `E0B368…423E` e commits `e727e9d`/`8db308d` |
| `evidence/EVD_POST_E13_SYMLINK_SANDBOX_DEFINITIVO_20260913.md` | ATIVO / EVIDÊNCIA `PASS_SANDBOX` | requalificação controlada definitiva r3 dos 31 testes de integração de symlink, 0 skips, 0 falhas/erros, hashes e regra de reexecução; nenhum symlink criado no host | governança, decisão de continuidade pós-E13, `integration_sync.py`, runner/Dockerfile hashados e controle de continuidade |
...
| `evidence/EVD_POST_E13_UNITY_CONTROLADO_SANDBOX_20260913.md` | ATIVO / EVIDÊNCIA `BLOCKED` | requalificações r9–r15 reais/preparadas em Windows Sandbox: r11 preserva a inconsistência do metadado de rede, r12/r13 registram rede habilitada de forma coerente, r14 iniciou o Unity e alcançou a compilação real com erros de referências dos módulos `ImageConversion`/`Animation`, e r15 expirou aguardando instalação/handoff manual sem iniciar Unity; a correção declarativa do pacote está aplicada e pendente de nova requalificação; shutdown da sandbox comprovado, shutdown limpo do Unity ainda pendente; r1–r10 preservados | governança, decisão de soak, `sandbox-attempt.json`, configurações/runner hashados e resultados/logs sanitizados, CHG de diagnósticos controlados e captura humana parcial hashada |
| `evidence/CHG_POST_E13_UNITY_SANDBOX_KEEPALIVE_20260913.md` | ATIVO / REGISTRO DE MUDANÇA `IN_PROGRESS` | causa comprovada do encerramento prematuro do r15, preservação das repetições e preparação do harness r16 sem timeout automático, com mount persistente dedicado; nenhuma execução Unity declarada | governança, `EVD_POST_E13_UNITY_CONTROLADO_SANDBOX_20260913.md`, r15 preservado, teste de contrato e checkpoint `8249718` |
| `evidence/AUDITORIA_FECHAMENTO_PENDENCIAS_POS_E13_20260913.md` | ATIVO / AUDITORIA DE FECHAMENTO — UNITY PENDENTE DE REQUALIFICAÇÃO E DECISÃO EXTERNA | matriz corrente de suíte, symlink controlado, responsividade aceita, memória/CUDA, CuPy, preservação, fluxo canônico e Unity; separa PASS, limitação, FAIL aceito, SKIP protegido, timeout e BLOCKED | governança, `CONTROLE_CONTINUIDADE_ATUAL.json`, evidências hashadas de soak/symlink/Unity e decisão formal do proprietário |
