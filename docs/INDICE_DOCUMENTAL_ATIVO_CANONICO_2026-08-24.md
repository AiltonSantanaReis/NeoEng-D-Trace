...
| `evidence/EVD_POST_E13_UNITY_CONTROLADO_SANDBOX_20260913.md` | ATIVO / EVIDÊNCIA `BLOCKED` | requalificação r8 real em Windows Sandbox: Unity iniciou e alcançou LicensingClient, mas código 198/token ausente/licença inválida impediu o pacote; shutdown da sandbox comprovado, shutdown limpo do Unity ainda pendente; r1–r7 preservados; candidato local de entitlement registrado somente por metadados, não utilizado | governança, decisão de soak, `sandbox-attempt.json` e resultado/log sanitizado hashados, CHG de diagnósticos controlados |
