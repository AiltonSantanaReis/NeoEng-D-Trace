...
| `evidence/EVD_POST_E13_OFFICIAL_SUITE_SAFE_20260913.md` | ATIVO / EVIDÊNCIA `PASS` | suíte oficial completa segura r4: 2630 pass, 2 skips controlados, 0 warnings; JUnit normalizado para higiene e failures históricos preservados | governança, guard de symlink, mudanças de higiene e controle de continuidade |
| `evidence/EVD_POST_E13_OFFICIAL_SUITE_PRIVACY_FAILURE_20260913.md` | ATIVO / EVIDÊNCIA `FAIL` PRESERVADA | falhas r5/r6 preservadas: higiene de caminho absoluto no JUnit/log e `WinError 5` intermitente no replace do atlas; raw/sanitizados hashados | governança, teste de higiene, exportação atômica e suítes oficiais r5/r6 |
| `evidence/CHG_POST_E13_CUPY_FALLBACK_LOG_20260913.md` | ATIVO / REGISTRO DE MUDANÇA `IN_PROGRESS` | ausência esperada do acelerador CuPy passa a INFO; falha anormal de driver continua WARNING e erro de processamento continua ERROR | governança, ADR CuPy, view_processor e suíte/build pós-E13 |
| `evidence/CHG_POST_E13_ATOMIC_REPLACE_RETRY_20260913.md` | ATIVO / REGISTRO DE MUDANÇA `IN_PROGRESS` | retry controlado para recusas transitórias do replace atômico do atlas, sem predelete e com rollback preservado | governança, finding WinError 5, testes atômicos e suíte oficial |
...
| `evidence/CHG_POST_E13_TZDATA_PACKAGING_20260913.md` | ATIVO / REGISTRO DE MUDANÇA `IN_PROGRESS` | dependência runtime pinada para eliminar o hidden import warning e tornar `ZoneInfo` reprodutível no Windows; build e smoke ainda são gates desta mudança | governança, `pyproject.toml`, `poetry.lock`, build portátil e smoke |
