...
| `evidence/EVD_POST_E13_UNITY_CONTROLADO_SANDBOX_20260913.md` | ATIVO / EVIDÊNCIA `BLOCKED` | requalificações r9–r12 reais em Windows Sandbox: r11 preserva a inconsistência do metadado de rede, r12 registrou rede habilitada de forma coerente; Unity alcançou LicensingClient, encontrou zero entitlements aplicáveis e saiu com código 198; shutdown da sandbox comprovado, shutdown limpo do Unity ainda pendente; r1–r10 preservados | governança, decisão de soak, `sandbox-attempt.json` e resultados/logs sanitizados hashados, CHG de diagnósticos controlados |
| `evidence/AUDITORIA_FECHAMENTO_PENDENCIAS_POS_E13_20260913.md` | ATIVO / AUDITORIA DE FECHAMENTO — UM BLOQUEIO EXTERNO RESTANTE | matriz corrente de suíte, symlink controlado, responsividade aceita, memória/CUDA, CuPy, preservação, fluxo canônico e Unity; separa PASS, limitação, FAIL aceito, SKIP protegido e BLOCKED | governança, `CONTROLE_CONTINUIDADE_ATUAL.json`, evidências hashadas de soak/symlink/Unity e decisão formal do proprietário |
