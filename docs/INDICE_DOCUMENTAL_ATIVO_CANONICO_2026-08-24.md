...
| `evidence/DECISAO_REVISAO_HUMANA_FINAL_POS_E13_20260911.md` | HISTÓRICO PRESERVADO / DECISÃO DEFERIDA | registro original da revisão humana final deferida; não é o estado vigente | governança, decisão de continuidade pós-E13 e evidência nativa profissional; superseded quanto ao estado por decisão de 2026-09-13 |
| `evidence/DECISAO_REVISAO_HUMANA_APROVADA_POS_E13_20260913.md` | ATIVO / DECISÃO `PASS` | aprovação humana explícita do proprietário sobre o pacote nativo pós-E13, sem encerrar gates técnicos ainda abertos | governança, decisão histórica de revisão humana, evidências nativas pós-E13 e controle de continuidade |
...
| `evidence/EVD_POST_E13_RUNTIME_REQUALIFICACAO_20260913.md` | ATIVO / REQUALIFICAÇÃO TÉCNICA — GATES POSITIVOS PASS; LOG UNITY PENDING_EVIDENCE; LOTE IN_PROGRESS | reexecução pós-commit dos runtimes Tilemap e híbrido em Godot/Unity, capturas reais, hashes, rejeição de drift e correção do status interno do harness; os diagnósticos nativos continuam preservados | governança, CHG de Tilemap, CHG híbrido, decisão de continuidade, decisão de revisão humana e commit `d7ddbef` |
| `evidence/AUDITORIA_POST_E13_PERFORMANCE_BASELINE_20260913.md` | ATIVO / AUDITORIA TÉCNICA — INVESTIGAÇÃO EVIDENCIADA; RESPONSIVIDADE EM ESCALA FAIL; MEMÓRIA PASS; GPU DO VIEWPORT NOT_APPLICABLE | medição estrutural em escala, perfis causais, correções estreitas de viewport, build portátil r3, evidência nativa do editor canônico 2D/3D/híbrido e soak controlado de memória; workload CUDA dedicado separado qualificado | governança, decisão O-2, decisão de continuidade pós-E13, commit `7f5c047` e build r3 |
| `evidence/EVD_POST_E13_PERFORMANCE_PENDENCIAS_20260913.md` | ATIVO / EVIDÊNCIA `IN_PROGRESS` | p95 residual de 249,28–398,73 ms em 512 objetos únicos, limite aceito formalmente, memória longa PASS no Docker, workload CUDA controlado PASS, GPU/frame do QGraphicsView NOT_APPLICABLE e Unity controlado BLOCKED | governança, auditoria de baseline, contrato O-2, decisão de soak e continuidade pós-E13 |
| `evidence/EVD_POST_E13_SOAK_MEMORIA_GPU_CONTROLADO_20260913.md` | ATIVO / EVIDÊNCIA `PASS` memória e workload CUDA controlado; viewport `NOT_APPLICABLE` | soak controlado de 26 workloads com 250 ciclos de memória por carga e workload CUDA de 20 s com 38 amostras, 36.408 operações e pico de 91% na RTX 3070 Ti; contador do QGraphicsView permanece não exposto | governança, decisão de soak, benchmark P2D-05, relatório GPU `E0B368…423E` e commits `e727e9d`/`8db308d` |
| `evidence/EVD_POST_E13_SYMLINK_SANDBOX_DEFINITIVO_20260913.md` | ATIVO / EVIDÊNCIA `PASS` no escopo da Sandbox | requalificação controlada definitiva r4 dos 31 testes de integração de symlink, 0 skips, 0 falhas/erros, hashes e regra de reexecução; nenhum symlink criado no host | governança, decisão de continuidade pós-E13, `integration_sync.py`, runner/Dockerfile hashados e controle de continuidade |
| `evidence/CHG_POST_E13_CONTROLLED_DANGEROUS_TESTS_20260913.md` | ATIVO / REGISTRO DE MUDANÇA — PASS | guard explícito para impedir symlink nativo e autorizar somente a execução controlada no sandbox | governança, evidência sandbox definitiva, testes de integração e controle de continuidade |
| `evidence/CHG_POST_E13_PATH_HYGIENE_20260913.md` | ATIVO / REGISTRO DE MUDANÇA — PASS | remoção de caminhos absolutos de usuário dos documentos rastreados, sem remoção de artefatos | governança, contrato de higiene e controle de continuidade |
| `evidence/EVD_POST_E13_OFFICIAL_SUITE_SAFE_20260913.md` | ATIVO / EVIDÊNCIA BASELINE `PASS` | suíte oficial segura r4 preservada: 2630 pass, 2 skips controlados, 0 warnings; a evidência corrente r8 está vinculada separadamente | governança, guard de symlink, mudanças de higiene e controle de continuidade |
| `evidence/EVD_POST_E13_OFFICIAL_SUITE_R8_20260913.md` | ATIVO / EVIDÊNCIA HISTÓRICA `PASS` | suíte oficial completa r8: 2633 pass, 2 skips controlados, 0 warnings; JUnit/log hashados e failures históricos preservados | governança, guard de symlink, higiene, retry atômico, CuPy logging e tzdata |
| `evidence/EVD_POST_E13_OFFICIAL_SUITE_R9_20260913.md` | ATIVO / EVIDÊNCIA HISTÓRICA `PASS` | suíte oficial completa r9: 2633 pass, 2 skips controlados, 0 warnings; JUnit/log/metadata hashados e failures históricos preservados | governança, guard de symlink, higiene, retry atômico, CuPy logging, tzdata e continuidade |
| `evidence/EVD_POST_E13_OFFICIAL_SUITE_R12_20260913.md` | ATIVO / EVIDÊNCIA CORRENTE `PASS` | suíte oficial completa r12: 2634 pass, 2 skips controlados, 0 warnings; JUnit/log/metadata sanitizados e hashados, failures de infraestrutura e tentativa de sanitização inválida preservados | governança, guard de symlink, higiene, retry atômico, CuPy logging, tzdata, memória Linux e continuidade |
| `evidence/EVD_POST_E13_SUITE_RECONCILIACAO_20260914.md` | ATIVO / EVIDÊNCIA CORRENTE `PASS` | requalificação integral corrente: 2641 pass, 2 skips controlados, 0 warnings; falha ambiental de tzdata e ocorrência intermitente de fingerprint preservadas, sem mascaramento | governança, reconciliação documental pós-E13, contrato de cache do viewport, tzdata e continuidade |
| `evidence/EVD_POST_E13_OFFICIAL_SUITE_PRIVACY_FAILURE_20260913.md` | ATIVO / EVIDÊNCIA `FAIL` PRESERVADA | falhas r5/r6/r7 preservadas: higiene de caminho absoluto no JUnit/log e `WinError 5` intermitente no replace do atlas; raw/sanitizados hashados | governança, teste de higiene, exportação atômica e suítes oficiais históricas |
| `evidence/CHG_POST_E13_CUPY_FALLBACK_LOG_20260913.md` | ATIVO / REGISTRO DE MUDANÇA `PASS` | ausência esperada do acelerador CuPy passa a INFO; falha anormal de driver continua WARNING e erro de processamento continua ERROR; build r5 e r9 comprovam os gates | governança, ADR CuPy, view_processor e suíte/build pós-E13 |
| `evidence/CHG_POST_E13_ATOMIC_REPLACE_RETRY_20260913.md` | ATIVO / REGISTRO DE MUDANÇA `PASS` | retry controlado para recusas transitórias do replace atômico do atlas, sem predelete e com rollback preservado; foco, repetição, r9 e build passaram | governança, finding WinError 5, testes atômicos e suíte oficial |
| `evidence/CHG_POST_E13_UNITY_DIAGNOSTICOS_CONTROLADOS_20260913.md` | ATIVO / REGISTRO DE MUDANÇA `PASS` | classificador somente leitura para licensing/shutdown, testes focados e execução Docker sem Unity nativo; sinais `Code 10`, token, Curl, `abort_threads` e `MemoryLeaks` preservados | governança, evidência Unity controlada, requalificação Unity histórica e controle de segurança |
| `evidence/EVD_POST_E13_UNITY_DIAGNOSTICOS_CONTROLADOS_20260913.md` | ATIVO / EVIDÊNCIA `PASS` DO CLASSIFICADOR; AMBIENTE LIMPO `PENDING_EVIDENCE` | classificação controlada dos logs Unity históricos, funcionalidade positiva/negativa observada e limites de licensing/shutdown sem nova execução nativa | governança, CHG de diagnósticos controlados, logs r2 e Docker Desktop |
| `evidence/EVD_POST_E13_UNITY_CONTROLADO_SANDBOX_20260913.md` | ATIVO / EVIDÊNCIA `IN_PROGRESS` | requalificações r9–r16 reais/preparadas em Windows Sandbox; r16 iniciou o Unity `6000.5.7f1`, validou o pacote e saiu `0`; licensing/shutdown limpos permanecem `BLOCKED`, todos os sinais e históricos são preservados | governança, decisão de soak, `sandbox-attempt.json`, configurações/runner hashados, resultados/logs sanitizados e CHG de diagnósticos controlados |
| `evidence/EVD_POST_E13_UNITY_R16_RESULTADO_20260914.md` | ATIVO / EVIDÊNCIA `IN_PROGRESS` | r16 corrente: execução real do Unity e pacote UPM `PASS`, entitlement observado `PASS`, licensing/shutdown limpos `BLOCKED`, Sandbox e host seguros `PASS`; importação do asset 3D no Unity permanece `PENDING_EVIDENCE` | governança, r16 hashado, relatório cumulativo do Sandbox, package report e asset Unity |
| `evidence/CHG_POST_E13_UNITY_SANDBOX_KEEPALIVE_20260913.md` | ATIVO / REGISTRO DE MUDANÇA `PASS` | causa comprovada do encerramento prematuro do r15, preservação das repetições e harness r16 sem timeout automático; comportamento foi exercitado no r16 | governança, `EVD_POST_E13_UNITY_CONTROLADO_SANDBOX_20260913.md`, r15/r16 preservados, teste de contrato e checkpoint `8249718` |
| `evidence/AUDITORIA_FECHAMENTO_PENDENCIAS_POS_E13_20260913.md` | ATIVO / AUDITORIA DE FECHAMENTO — GATES UNITY ESPECÍFICOS `BLOCKED` | matriz corrente de suíte, symlink controlado, responsividade aceita, memória/CUDA, CuPy, preservação, fluxo canônico e Unity; separa PASS, limitação, FAIL aceito, testes `skipped` protegidos e BLOCKED | governança, `CONTROLE_CONTINUIDADE_ATUAL.json`, evidências hashadas de soak/symlink/Unity e decisão formal do proprietário |
| `evidence/PLANO_ASSET_3D_UNITY_REFERENCIA_20260914.md` | ATIVO / PLANO `IN_PROGRESS` | plano do asset modular Eclipse Warden, exportação, proveniência, validação e limites de importação Unity | governança, decisão pós-E13, referências fornecidas pelo proprietário e contrato Unity |
| `evidence/RELATORIO_ASSET_3D_UNITY_20260914.md` | ATIVO / RELATÓRIO `IN_PROGRESS` | asset r3 com GLB/OBJ, 23 componentes, PBR, UV, skin e animação; validação estrutural e preview Godot `PASS`; importação nativa Unity `PENDING_EVIDENCE` | governança, plano do asset, manifest/hash e preview Godot |
| `evidence/ADR_POST_E13_CUPY_AVALIACAO_20260913.md` | ATIVO / ADR `PASS`; ADOÇÃO OFICIAL `NOT_APPLICABLE` | avaliação real do caminho opcional CuPy/X-Ray, microbenchmark, fallback, impacto de empacotamento e decisão de não promover a dependência | governança, auditoria de desempenho pós-E13, `pyproject.toml`, spec portátil e artefato CuPy |
| `evidence/CHG_POST_E13_TZDATA_PACKAGING_20260913.md` | ATIVO / REGISTRO DE MUDANÇA `PASS` | dependência runtime pinada, contrato `ZoneInfo`, suíte r9 e build r5 com hook carregado e sem hidden import ausente | governança, `pyproject.toml`, `poetry.lock`, build portátil e smoke |
| `evidence/EVD_POST_E13_BUILD_CUPY_ATOMIC_20260913.md` | ATIVO / EVIDÊNCIA DE BUILD `PASS` | build portátil r5 limpa, executável/ZIP hashados, smoke 11/11, ciclo GUI real PT, CuPy opcional, retry atômico e tzdata qualificados | governança, mudanças CuPy/atlas/tzdata, provenance e suíte r9 |
