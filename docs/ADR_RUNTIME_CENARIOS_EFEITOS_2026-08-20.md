...
A RUNTIME-ETAPA-6 foi integrada no merge 46604d3 para implementar triggers
versionados, zonas, condições, prioridades, eventos, cancelamento e replay
determinístico. A capacidade está aprovada somente no escopo do dispatcher CPU
determinístico e não declara suporte de engine ou release.

### Estado vivo da RUNTIME-ETAPA-7 — em desenvolvimento

A baseline de caracterização antes da alteração foi o HEAD `1e6b4c8f08f2237ec4dcc2ff8a5c6368515be1c9`, com baseline Git-blob e evidências verificadas antes da criação da branch. A implementação atual adiciona o sidecar versionado `runtime.streaming`, com hashes de assets, caminhos relativos POSIX, limites de bytes, prioridades, carregamento assíncrono sob raiz confinada, cache LRU, descarte seguro, cancelamento observável, retry explícito e recuperação após falhas.

A auditoria local reproduziu todos os checks funcionais da fase — round-trip canônico, hash de origem, leitura real, ordem por prioridade, limite de pendências, cancelamento, reativação, eviction, falha de hash, retry, limite de cache, persistência atômica, limites e capacidade anunciada — mas registrou `FAIL` exclusivamente porque a árvore estava modificada (`source_tree_clean=false`). Esse resultado é esperado durante o desenvolvimento e não constitui aprovação. A fase somente poderá ser promovida após auditoria em árvore limpa, testes integrais, cobertura sem redução, bytes Git, evidências hashadas, CI Linux/Windows, PR e validação pós-merge.

A correção aplicada após a reprodução inicial removeu a transição incorreta que cancelava assets pendentes sem requisição e permitiu reativação determinística após `CANCELLED`/`EVICTED`. O caso foi transformado em teste regressivo; nenhuma regressão foi declarada sem reprodução.
