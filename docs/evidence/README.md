...
Estado vivo em 20 de agosto de 2026: as Etapas 1, 2 e 3 estão encerradas nos merges `84dfee7`/`7e190b4`, `eb9837b`/`ff66fa7` e `c76ac4b`, respectivamente, com CI Linux/Windows e validação pós-merge documentados. A RUNTIME-ETAPA-5 está encerrada no merge `159b1241b012` com documentação pós-merge registrada; a RUNTIME-ETAPA-6 está em desenvolvimento local e ainda não está aprovada ou integrada. A Etapa 3 foi aprovada somente no escopo de contrato de shaders, compilação real via Qt Shader Tools, publicação atômica, rollback e rejeição explícita de backends não declarados. Isso não constitui aprovação automática de release.
