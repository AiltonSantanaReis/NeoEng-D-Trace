## Registro corrente — pós-merge em `main` — 06/09/2026

A PR `#171` foi integrada no merge commit
`18a82bcab0b9319ede29e05b39673e7dbf066e63`, originado do head
`ab0753fcc45d466e4c571d7d9603c9ef3703b6b3`. O CI remoto `34025199115` passou
em Linux e Windows. A validação independente do conteúdo integrado passou
foco `66/66`, suíte completa `2024 passed, 2 skipped, 1 warning`, baseline
`3264 files` e `135` manifestos de evidências.

A build portátil correspondente está em
`build/release-postmerge-18a82bc-20260906`; o manifesto declara o merge
commit, `314` arquivos e smoke `SUCCESS`. O executável tem SHA-256
...
Os dois skips de symlink continuam explicitamente limitados ao privilégio
Windows. Tag e release não fazem parte deste registro.
...
- ETAPA_2_INTERFACE_MODERNA_ENCERRAMENTO_POS_MERGE_2026-08-21.md — PR #133, merge ff66ffd, CI Linux/Windows, baseline/evidência Git-blob e suíte pós-merge aprovados no escopo da etapa.
- `ETAPA_2_ICONES_DPI_2026-08-23.md` — matriz Qt 100/125/150/200%, galeria completa 16/20/24/32 px, hashes, clipping, auditoria visual, PR #150, CI Linux/Windows e validação pós-merge no merge `bcb0951ee05c41b03eae2a66e712d1f041cde7f8`; aprovada somente no escopo comprovado.
...
- `ETAPA_5_RUNTIME_POS_PROCESSAMENTO_2026-08-20.md` — evidência pré-merge da
  implementação local e testes pré-merge; consulte também `ETAPA_5_RUNTIME_POS_PROCESSAMENTO_ENCERRAMENTO_POS_MERGE_2026-08-20.md`, que registra o PR #121, merge 159b1241b012, CI Linux/Windows e validação pós-merge PASS.
- `ETAPA_6_RUNTIME_TRIGGERS_2026-08-20.md` — evidência pré-merge da implementação local de triggers determinísticos; não declara integração, CI, merge ou suporte de engine.
...
Plano vivo da modernização da interface: docs/PLANO_INTERFACE_MODERNA_PROFISSIONAL_2026-08-21.md. Este é o documento fonte de verdade somente para esse escopo de UI, subordinado às políticas globais. As Etapas 0–9 estão reconciliadas no estado técnico automatizado; revisão visual humana, CI dos commits locais e aprovação de release continuam separados. A matriz e os limites estão em docs/evidence/REMEDIACAO_ACHADOS_AUDITORIA_ETAPAS_0_9_2026-08-23.md.
...
- ETAPA_6_CENARIOS_PROFISSIONAIS_2026-08-20.md — testes reais, auditoria visual automatizada, desempenho, hashes e privacidade; PR 110, CI 32336447483 e merge ecbaab6 aprovados.
