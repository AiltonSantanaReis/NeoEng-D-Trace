...
Estado vivo em 20 de agosto de 2026: as Etapas 1, 2 e 3 estão encerradas nos merges `84dfee7`/`7e190b4`, `eb9837b`/`ff66fa7` e `c76ac4b`, respectivamente, com CI Linux/Windows e validação pós-merge documentados. A RUNTIME-ETAPA-5 está encerrada no merge `159b1241b012` com documentação pós-merge registrada; a RUNTIME-ETAPA-6 está encerrada no escopo do dispatcher CPU determinístico pelo merge `46604d336af7867e0dd59f9af6e07e5b39a5827f`, com CI da PR, auditoria e validação local pós-merge documentados. A RUNTIME-ETAPA-7 está em desenvolvimento na branch `Ailton/runtime-stage7-streaming`, com auditoria funcional local reproduzida, mas ainda não aprovada porque a árvore está modificada e faltam os gates de publicação. A Etapa 3 foi aprovada somente no escopo de contrato de shaders, compilação real via Qt Shader Tools, publicação atômica, rollback e rejeição explícita de backends não declarados. Isso não constitui aprovação automática de release.
