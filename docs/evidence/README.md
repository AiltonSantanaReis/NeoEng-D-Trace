...
A Etapa 2 do ADR de runtime (iluminação e materiais) está em validação remota na PR #115 e ainda não está aprovada. O primeiro CI falhou no isort e o segundo falhou na cobertura de branches (`84,94% < 85,00%`); os fixes `43733cb` e `4038bac` passaram a validação local, e o pacote r3 está registrado. A PR aguarda nova execução remota. Nenhuma capacidade de runtime será considerada concluída antes dos gates próprios.
