...
A baseline final foi regenerada e aprovada com 306 arquivos.

## CI pré-merge auditado

O workflow `31476442683` executou sobre o HEAD fonte
`c9bafdd75a74a8dae39d814fd6b0ccf35a2f9f96`.

- Linux: job `93731130311`, artefato `9095447008`, SHA-256
  `9c43f7c79f53dad7253ffb8f21b0a399bb866fe2bf0407efea356d66b84f568b`;
- Windows: job `93731130325`, artefato `9095475339`, SHA-256
  `81641c9e0e2b7ee071ed4009f7c2305dc02af6f4a6dee10161c9996b2993ee06`;
- merge sintético testado `ee5a5ca966fa343e13a2a00acb5cf7569afdb890`,
  com pais base `5e88c8d548e2b60612601f83e1bf24aeb91081bb` e fonte
  `c9bafdd75a74a8dae39d814fd6b0ccf35a2f9f96`;
- `754 passed` nos dois sistemas, com cobertura semanticamente idêntica:
  `9.314/11.632` linhas e `2.453/3.704` ramos;
- legado Windows: 196 testes, reconciliação `27/27`, zero inesperadas, zero
  ausentes, zero erros e árvore limpa;
- 54 arquivos externos, 48 evidências e 1.413 payloads recursivos inspecionados;
- 327 checksums internos validados e zero referências proibidas, caminhos
  pessoais ou divergências de checksum;
- os 42 arquivos textuais de evidência diferiram dos blobs apenas por CRLF no
  checkout Windows; após normalização canônica de fim de linha, as 48
  evidências coincidiram com o commit. Nenhuma outra diferença foi aceita.

O CI pré-merge foi **ACEITO** após essa inspeção. Isso não comprova integração,
CI pós-merge, encerramento de `R-003`, conclusão da Etapa 11 nem release.
...
- O CI pré-merge do Pacote 2 foi aceito; integração e CI pós-merge não existem.
...
TECHNICAL_COMMIT=c9bafdd75a74a8dae39d814fd6b0ccf35a2f9f96
