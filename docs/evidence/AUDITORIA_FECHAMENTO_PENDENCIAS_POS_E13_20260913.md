...
- O r14 preserva os cinco grupos de erros de compilação observados: `Texture2D.LoadImage`
  ...
  `package.json`, e foi validada no método do pacote por uma nova execução real
  do Unity no r16. O r15 preserva o timeout `124` aguardando a instalação/handoff
...
contrato do pacote; a declaração dos módulos necessários já foi corrigida no
  ...
  instalação/handoff manual e não substitui o r16. Resta uma decisão de escopo
  para os gates residuais:
