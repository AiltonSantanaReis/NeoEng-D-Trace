...
def rdp_simplify(
    ...
    def point_line_distance(p, p1, p2):
        ...
        num = abs(
            (p2[0] - p1[0]) * (p1[1] - p[1]) - (p1[0] - p[0]) * (p2[1] - p1[1])
        )
    ...
    for curvature in curvatures:
        if max_curvature > 0:
            ...
            adaptive_eps = base_eps * (
                1.0 + curvature_factor * (1.0 - normalized_curv)
            )
        else:
            adaptive_eps = base_eps
...
def _iterative_rdp_with_weights(
    ...
    current_weights = list(weights) if len(weights) == len(points) else [1.0] * len(points)
