...
class TransformGizmo:
    ...
    def hit_test(self, mouse_pos: QPointF) -> int:
        ...
        if self._distance_to_segment(mouse_pos, self.screen_pos, x_end) <= self.handle_thickness:
            return self.AXIS_X

        y_end = QPointF(self.screen_pos.x(), self.screen_pos.y() - self.arm_length)
        if self._distance_to_segment(mouse_pos, self.screen_pos, y_end) <= self.handle_thickness:
            return self.AXIS_Y

        # Keep the old downward Y hit area for existing integrations while
        # rendering the reference orientation upward.
        y_legacy_end = QPointF(self.screen_pos.x(), self.screen_pos.y() + self.arm_length)
        if self._distance_to_segment(mouse_pos, self.screen_pos, y_legacy_end) <= self.handle_thickness:
            return self.AXIS_Y
    ...
    def _color(self, operation: int, base: QColor) -> QColor:
        color = QColor(self.color_hover if operation == self.hover_axis or operation == self.active_axis else base)
    ...
    def _draw_arrow(painter: QPainter, end: QPointF, direction: QPointF, size: float):
        ...
        painter.drawPolygon(QPolygonF([end, base + perpendicular * (size / 2.6), base - perpendicular * (size / 2.6)]))
    ...
    def draw(self, painter: QPainter):
        ...
        painter.drawEllipse(-self.rotation_radius, -self.rotation_radius, self.rotation_radius * 2, self.rotation_radius * 2)
        ...
        painter.drawRect(-self.plane_size, -self.plane_size, self.plane_size, self.plane_size)

        x_color = self._color(self.AXIS_X, self.color_x)
        painter.setPen(QPen(x_color, 3.0, Qt.PenStyle.SolidLine, Qt.PenCapStyle.RoundCap))
        painter.drawLine(QPointF(0, 0), QPointF(self.arm_length, 0))
        self._draw_arrow(painter, QPointF(self.arm_length + self.arrow_size, 0), QPointF(1, 0), self.arrow_size)
        painter.setPen(QPen(self._color(self.SCALE_X, self.color_x), 2.0))
        painter.setBrush(QBrush(QColor(20, 20, 20, 220)))
        painter.drawRect(self.arm_length - self.scale_handle_size / 2, -self.scale_handle_size / 2, self.scale_handle_size, self.scale_handle_size)

        y_color = self._color(self.AXIS_Y, self.color_y)
        painter.setPen(QPen(y_color, 3.0, Qt.PenStyle.SolidLine, Qt.PenCapStyle.RoundCap))
        painter.drawLine(QPointF(0, 0), QPointF(0, -self.arm_length))
        self._draw_arrow(painter, QPointF(0, -self.arm_length - self.arrow_size), QPointF(0, -1), self.arrow_size)
        painter.setPen(QPen(self._color(self.SCALE_Y, self.color_y), 2.0))
        painter.setBrush(QBrush(QColor(20, 20, 20, 220)))
        painter.drawRect(-self.scale_handle_size / 2, -self.arm_length - self.scale_handle_size / 2, self.scale_handle_size, self.scale_handle_size)
        ...
        painter.drawEllipse(-self.center_radius, -self.center_radius, self.center_radius * 2, self.center_radius * 2)
