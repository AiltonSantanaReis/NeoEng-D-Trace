...
class ClipBlock(QGraphicsRectItem):
    def __init__(self, panel, clip, row):
        ...
        self.setPen(
            QPen(QColor("#53d4e8" if panel.selected_id == clip.id else "#b6c2ce"))
        )
        self.setToolTip(
            f"{clip.name} · {clip.start:.2f}s — {clip.start + clip.duration:.2f}s"
        )
...
class SequenceViewport(SceneAuthoringViewport):
    ...
    def paintEvent(self, event):
        ...
        for clip in self.sequence_clips:
            if clip.layer_id and not next(
                (
                    layer.visible
                    for layer in document.layers
                    if layer.id == clip.layer_id
                ),
                False,
            ):
                continue
            ...
            if clip.kind == "text":
                box = QRectF(
                    24, self.viewport().height() - 112, self.viewport().width() - 48, 88
                )
            ...
            for particle in particles_at(clip, self.sequence_position):
                ...
                point = self.mapFromScene(
                    self._project_position(
                        Point3Record(x=particle.x, y=particle.y, z=0),
                        clip.layer_id or document.layers[0].id,
                    )
                )
                ...
                if clip.kind == "rain":
                    painter.drawLine(
                        ...
                        point.y(),
                        point.x() - 2,
                        point.y() + int(particle.size),
                    )
                else:
                    painter.drawEllipse(
                        point, max(1, int(particle.size)), max(1, int(particle.size))
                    )
        painter.end()
...
class SceneSequencePanel(QWidget):
    ...
    def __init__(
        self, session, viewport, pages, project_root, language="en", parent=None
    ):
    ...
    def update_language(self, language):
        ...
        for widget, label in (
            (
                (self.play, "Pausar" if self.timer.isActive() else "Reproduzir"),
                (self.stop_button, "Parar"),
                (self.add_button, "Adicionar"),
                (self.delete_button, "Excluir clipe"),
                (self.apply_button, "Aplicar clipe"),
                (self.loop, "Repetir"),
                (self.undo_button, "Desfazer"),
                (self.redo_button, "Refazer"),
            )
            if pt
            else (
                (self.play, "Pause" if self.timer.isActive() else "Play"),
                (self.stop_button, "Stop"),
                (self.add_button, "Add"),
                (self.delete_button, "Delete clip"),
                (self.apply_button, "Apply clip"),
                (self.loop, "Loop"),
        ...
        self.duration.setToolTip(
            "Duração da cena (segundos)" if pt else "Scene duration (seconds)"
        )
    ...
    def _error(self, exc):
        self.status_message.emit(
            (
                "Sequência não alterada. Verifique duração, referências e "
                "sobreposição de clipes: "
                if self.language == "pt"
                else "Sequence unchanged. Check duration, references and "
                "overlapping clips: "
            )
            + str(exc)
        )
    ...
    def add_clip(self, kind):
        ...
        try:
            values = dict(
                id=uuid4().hex,
                name=(NAMES_PT if self.language == "pt" else NAMES_EN)[
                    KINDS.index(kind)
                ],
                kind=kind,
                start=min(self.position, max(0, self.sequence.duration - 0.05)),
                duration=min(5, self.sequence.duration - self.position),
                loop=kind in {"rain", "snow", "dust", "fire", "audio"},
                layer_id=self.viewport._destination_layer(),
            )
            if kind == "camera":
                ...
                values.update(
                    x=camera.position.x,
                    y=camera.position.y,
                    end_x=camera.position.x + 200,
                    end_y=camera.position.y,
                    zoom=camera.zoom,
                    end_zoom=camera.zoom,
                    rotation=camera.rotation,
                    end_rotation=camera.rotation,
                )
            if kind == "motion":
                obj = next(
                    (
                        o
                        for o in self.session.document.objects
                        if o.id == self.session.selection.primary
                    ),
                    None,
                )
                if obj is None:
                    raise ValueError(
                        "Selecione o objeto a animar"
                        if self.language == "pt"
                        else "Select an object to animate"
                    )
                values.update(
                    target_id=obj.id,
                    x=obj.transform.position.x,
                    y=obj.transform.position.y,
                    end_x=obj.transform.position.x + 100,
                    end_y=obj.transform.position.y,
                    zoom=obj.transform.scale.x,
                    end_zoom=obj.transform.scale.x,
                    rotation=obj.transform.rotation.z,
                    end_rotation=obj.transform.rotation.z,
                )
            if kind == "text":
                values["text"] = (
                    "Sua história começa aqui"
                    if self.language == "pt"
                    else "Your story starts here"
                )
            if kind == "fire":
                values["color"] = "#ff9c40"
            if kind == "audio":
                path, _ = QFileDialog.getOpenFileName(
                    self,
                    "Adicionar áudio" if self.language == "pt" else "Add audio",
                    "",
                ...
                prepared = prepare_scene_asset(
                    path, self.project_root, allow_audio=True
                )
                asset = AssetReferenceRecord(
                    id="audio_" + prepared.sha256[:20],
                    path=prepared.path,
            ...
            sequence = SceneSequence.model_validate(
                {
                    **self.sequence.model_dump(),
                    "clips": [c.model_dump() for c in self.sequence.clips]
                    + [clip.model_dump()],
                }
            )
            if kind == "audio":
    ...
    def select_clip(self, clip_id, *, redraw=True):
        ...
        if clip:
            ...
            if clip.kind == "motion":
                relevant.update(
                    {"target", "rotation", "end_rotation", "opacity", "end_opacity"}
                )
            if clip.kind in {"light", "rain", "snow", "dust", "fire"}:
                relevant.update(
                    {"layer", "x", "y", "zoom", "color", "intensity", "loop", "opacity"}
                )
            if clip.kind == "audio":
    ...
    def apply_clip(self):
        ...
        values.update(
            ...
            loop=self.repeat.isChecked(),
            enabled=self.enabled.isChecked(),
            target_id=self.target.currentData(),
            layer_id=self.layer.currentData(),
        )
    ...
    def change_clip(self, clip_id, changes):
        ...
        try:
            data = self.sequence.model_dump()
            data["clips"] = [
                {**c.model_dump(), **changes} if c.id == clip_id else c.model_dump()
                for c in self.sequence.clips
            ]
            set_sequence(self.session, SceneSequence.model_validate(data))
    ...
    def remove_selected(self):
        self.stop()
        sequence = self.sequence.model_copy(
            update={
                "clips": [c for c in self.sequence.clips if c.id != self.selected_id]
            }
        )
        set_sequence(self.session, sequence)
    ...
    def draw_timeline(self):
        ...
        used = list(dict.fromkeys(c.kind for c in self.sequence.clips)) or [
            "camera",
            "audio",
            "text",
        ]
        for row, kind in enumerate(used):
            self.scene.addRect(
                ...
                31,
                QPen(QColor("#394552")),
                QBrush(QColor("#1d252e")),
            )
            label = self.scene.addText(
                (NAMES_PT if self.language == "pt" else NAMES_EN)[KINDS.index(kind)]
            )
        ...
        self.playhead = self.scene.addLine(
            0, 0, 0, self.scene.height(), QPen(QColor("#55d5e8"), 2)
        )
        self._display_time()

    def _display_time(self):
        self.time_label.setText(
            f"{self.position:06.2f} / {self.sequence.duration:.2f} s"
        )
    ...
    def seek(self, position):
        ...
        if self.preview is None:
            preview_session = SceneAuthoringSession(
                SceneAuthoringModel(self.session.document.model_copy(deep=True))
            )
            self.preview = SequenceViewport(
                preview_session, self.project_root, self.pages
            )
        ...
        self.preview.session.model.document = evaluate_sequence(
            self.session.document, self.position
        )
    ...
    def _sync_audio(self):
        active = {
            c.id: c
            for c in active_clips(self.sequence, self.position)
            if c.kind == "audio"
        }
        ...
        for clip_id, clip in active.items():
            ...
            if clip_id not in self.players:
                asset = next(
                    (a for a in self.session.document.assets if a.id == clip.asset_id),
                    None,
                )
                if asset is None:
                    ...
                    self.status_message.emit(
                        (
                            "Áudio ausente/alterado; revincule na Biblioteca: "
                            "asset não encontrado"
                            if self.language == "pt"
                            else "Missing/changed audio; relink in Library: "
                            "asset not found"
                        )
                    )
                    continue
                ...
                player.errorOccurred.connect(
                    lambda _error, message: self.status_message.emit(
                        (
                            "Falha no áudio; verifique o arquivo: "
                            if self.language == "pt"
                            else "Audio failed; check the file: "
                        )
                        + message
                    )
                )
