...
def prepare_scene_asset(
    source_path: str | os.PathLike[str], project_root: Path, *, allow_audio: bool = False
) -> PreparedSceneAsset:
    ...
    if allow_audio and candidate.suffix.lower() in {".wav", ".mp3", ".ogg", ".flac"}:
        if not candidate.is_file() or not 0 < candidate.stat().st_size <= 256 * 1024 * 1024:
            raise SceneAssetError("Audio must be a readable file between 1 byte and 256 MiB")
        source = candidate
    else:
