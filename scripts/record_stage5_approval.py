"""Record human approval and exact CI evidence for Stage 5."""
...
def main() -> int:
    ...
    if not commit or args.ci_commit != commit:
        raise SystemExit(f"CI commit diverges from report commit: {args.ci_commit} != {commit}")
    if report.get("current_contract_result") != "PASS":
        raise SystemExit("Stage 5 contract is not PASS")
    if any(check.get("result") != "PASS" for check in report.get("checks", [])):
        raise SystemExit("Stage 5 contains a failing check")

    report["human_review"] = {
        "decision": "APROVADO",
        "reviewer": args.reviewer,
        "recorded_at_utc": datetime.now(timezone.utc).isoformat(),
        "snapshot": f"STAGE_5_SNAPSHOT:{commit}",
    }
    ...
    manifest["stage5_report"]["sha256"] = report_hash
