...
print(
    f"SOURCE_SHA_PRESENT={int('7df73f21f5a609196e6cadac85065c89a989ecb4' in all_text)}"
)
forbidden_placeholder_count = len(
    re.findall(r"\b(?:TODO|TBD|PLACEHOLDER)\b", all_text, re.I)
)
...
numbered_paragraphs = sum(
    p._p.pPr is not None and p._p.pPr.numPr is not None for p in doc.paragraphs
)
print(f"NUMBERED_PARAGRAPHS={numbered_paragraphs}")
...
print(f"MARGINS_DXA={','.join(str(value) for value in margin_dxa)}")
print(
    f"HEADER_FOOTER_DXA={section.header_distance.twips},{section.footer_distance.twips}"
)
print(
    "TABLE_WIDTHS_DXA="
    + ",".join(
        str(sum(int(col.get(qn("w:w"))) for col in table._tbl.tblGrid.gridCol_lst))
...
nonempty_table_cells = sum(
    bool(cell.text.strip())
    for table in doc.tables
    for row in table.rows
    for cell in row.cells
)
print(f"NONEMPTY_TABLE_CELLS={nonempty_table_cells}")
