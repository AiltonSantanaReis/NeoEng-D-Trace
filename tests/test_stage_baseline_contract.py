...
BASELINE = (
    Path(__file__).parents[1] / "docs" / "evidence" / "final-target-baseline-v1.json"
)
...
def _report(
    tmp_path: Path,
    *,
    classification: str = "EXPECTED_EVOLUTION",
    sha: str | None = None,
) -> Path:
        ...
        "checks": [
            {
                ...
                "justification": (
                    "Token contract remains valid; geometry change is documented."
                ),
            }
        ],
        "artifacts": [
            {"path": artifact.name, "sha256": sha or actual_sha, "visual": False}
        ],
    }
...
def test_contract_accepts_classified_evolution_with_valid_hash(tmp_path: Path) -> None:
    errors = validate(
        load_json(BASELINE), load_json(_report(tmp_path)), _report(tmp_path).resolve()
    )
    assert errors == []
...
def test_contract_rejects_unclassified_change(tmp_path: Path) -> None:
    ...
    errors = validate(
        load_json(BASELINE), load_json(report_path), report_path.resolve()
    )
