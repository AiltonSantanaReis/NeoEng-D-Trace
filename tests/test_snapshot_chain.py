...
def test_root_snapshot_requires_no_parent_and_validates_hashes(tmp_path: Path) -> None:
    ...
    report.write_text(json.dumps({"baseline_id": "FINAL_TARGET", "parent_snapshot_id": None}), encoding="utf-8")
...
def test_root_snapshot_rejects_parent(tmp_path: Path) -> None:
    ...
    report.write_text(json.dumps({"baseline_id": "FINAL_TARGET", "parent_snapshot_id": "unexpected"}), encoding="utf-8")
