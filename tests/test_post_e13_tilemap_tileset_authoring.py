...
def test_tilemap_canvas_renders_loaded_texture_instead_of_only_placeholder_color(
    ...
    panel = TileMapAuthoringPanel(tmp_path)
    panel.new_map()
    # Capture the canvas as it is used in the editor: a child widget must have
    # its authoring panel visible so offscreen Qt cannot clip the grab to a
    # stale hidden-parent backing store after another Qt fixture ran.
    panel.show()
    panel.canvas.show()
    _app().processEvents()
    center = panel.canvas.rect().center()
    visible_cell = panel.canvas._cell_at(center)
